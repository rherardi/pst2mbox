// PST2MBOXApp.cpp: implementation of the CPST2MBOXApp class.
//
//////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
CPST2MBOXApp::CPST2MBOXApp() :
m_pstProc(new CPSTProc()), m_xmlProc(new CXMLMailProc()), m_verbose(false), m_printFormsOnly(false)
{
}
...
_TCHAR *CPST2MBOXApp::PrefixWithCurDir(_TCHAR *fileName, _TCHAR *fileNameFull)
{
...
int CPST2MBOXApp::ProcessCommandLine(int argc, _TCHAR *argv[])
{
...
CMailbox* CPST2MBOXApp::getMailboxInfo()
{
	return m_xmlProc->getMailboxInfo();
}

CMessageItem* CPST2MBOXApp::GetNextMessageItem(_TCHAR *partCondition)
{
	return m_xmlProc->GetNextMessageItem(partCondition);
}

CEntryTransformer* CPST2MBOXApp::getNextEntryTransformer(_TCHAR *nodeSelector)
{
	return m_xmlProc->getNextEntryTransformer(nodeSelector);
}

void CPST2MBOXApp::resetConversionRuleIndex()
{
	m_xmlProc->resetConversionRuleIndex();
}

void CPST2MBOXApp::resetEntryListIndex()
{
	m_xmlProc->resetEntryListIndex();
}

void CPST2MBOXApp::resetTransformerListIndex()
{
	m_xmlProc->resetTransformerListIndex();
}

void CPST2MBOXApp::InitializeMessageItemList()
{
	m_xmlProc->InitializeMessageItemList();
}

CConversionRuleEntry* CPST2MBOXApp::getNextConversionRuleEntry(_TCHAR *nodeSelector)
{
	return m_xmlProc->getNextConversionRuleEntry(nodeSelector);
}

const _TCHAR* CPST2MBOXApp::getConfigSettingValue(const _TCHAR* settingName)
{
	return m_xmlProc->getConfigSettingValue(settingName);
}

CXMLNameValue* CPST2MBOXApp::getNextNameValue(_TCHAR* selector, const _TCHAR *name_attr, const _TCHAR *value_attr)
{
	return m_xmlProc->getNextNameValue(selector, name_attr, value_attr);
}

bool CPST2MBOXApp::matchesRegExp(char *regExpPattern, char* matchString)
{
	return m_xmlProc->matchesRegExp(regExpPattern, matchString);
}

bool CPST2MBOXApp::IsSMTPLookupRequested()
{
	return m_xmlProc->IsSMTPLookupRequested();
}

int CPST2MBOXApp::ExtractSchemaFile(_TCHAR* tempDir)
{
	_TCHAR usage[256];
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	_TCHAR current_method[] = _TEXT("CPST2MBOXApp::ExtractSchemaFile");
	if (IsVerbose()){
...
int CPST2MBOXApp::StartMigration(_TCHAR* exec)
{
	_TCHAR usage[256];
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	_TCHAR current_method[] = _TEXT("CPST2MBOXApp::StartMigration");
	if (IsVerbose()){
		::LoadString(hInst, IDS_ENTER_METHOD, usage, 256);
		::_tprintf(_TEXT("%s %s\n"), usage, current_method);
	}
...
int wmain(int argc, _TCHAR* argv[])
{
	...
	CPST2MBOXApp *theApp = new CPST2MBOXApp();
